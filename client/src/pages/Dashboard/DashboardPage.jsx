...
const KPICard = ({
  ...
  return (
    <div
      className={`bg-white p-4 sm:p-5 lg:p-6 rounded-xl shadow-sm border border-[#D7CCC8] transition-all duration-200 hover:shadow-md ${
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
        <div
          className={`w-10 h-10 sm:w-11 sm:h-11 lg:w-12 lg:h-12 rounded-lg flex items-center justify-center flex-shrink-0 ${colorClasses[color]}`}
        >
          <Icon size={20} className="text-white sm:w-5 sm:h-5 lg:w-6 lg:h-6" />
        </div>
        <span className="text-xs sm:text-sm font-medium text-[#5D4037] leading-tight">
          {title}
        </span>
      </div>
      <div className="text-2xl sm:text-3xl font-bold text-[#3E2723] mb-1 sm:mb-2">
        {value}
      </div>
      {trend && (
...
const ActionTile = ({ icon: Icon, label, onClick }) => (
  ...
  >
    ...
    <span className="text-xs sm:text-sm font-medium text-[#3E2723] text-center leading-tight">
      {label}
    </span>
  </button>
)
  ...
  return (
    <div className="p-4 sm:p-5 bg-[#FBF8F4] border border-[#D7CCC8] rounded-lg hover:shadow-sm transition-shadow">
      <div className="flex items-center gap-2 mb-3 sm:mb-4">
        <Warehouse
          size={18}
          className="text-[#5D4037] sm:w-5 sm:h-5 flex-shrink-0"
        />
      ...
      <div className="flex justify-between gap-4 mb-3">
        <div className="flex-1">
          ...
          <div className="text-lg sm:text-xl font-semibold text-[#3E2723]">
...
const DashboardPage = () => {
  ...
  // Assumes your FastAPI is running on localhost:8000
  ...
  const loadDashboardData = async () => {
    setLoading(true)
    try {
      // 1. Fetch Calculated KPIs from Dashboard Manager
      const kpiResponse = await fetch(`${API_BASE_URL}/dashboard/kpis`)
      ...
      // 3. Fetch Products & Stock to calculate total units and warehouse utilization
      // logic: /products/ returns [products[], stock[]]
      const prodResponse = await fetch(`${API_BASE_URL}/products/`)
      const prodData = await prodResponse.json()
      const allStock = prodData[1] || [] // Index 1 is the stock list

      // Calculate total stock units (sum of on_hand)
      const totalStockUnits = allStock.reduce(
        (sum, item) => sum + (item.on_hand || 0),
        0
      )

      // Map stock to warehouses for the Warehouse Cards
      const warehouseStats = warehousesData.map((wh) => {
        const warehouseStock = allStock
        ...
        return {
          ...
          // Fallback capacity if not in DB model
          capacity: wh.capacity || 10000,
        }
      })

      setDashboardData({
        productCount: kpiData.total_products,
        totalStockUnits: totalStockUnits,
        lowStockCount: kpiData.low_stock_items,
        pendingReceiptsCount: kpiData.pending_receipts,
        pendingDeliveriesCount: kpiData.pending_deliveries,
        scheduledTransfersCount: kpiData.internal_transfers,
    ...
    return (
      <div className="flex items-center justify-center min-h-[calc(100vh-64px)]">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-base sm:text-lg text-[#5D4037] font-medium">
            Loading dashboard...
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4 lg:gap-5 mb-6 sm:mb-8">
          ...
          <KPICard
            ...
            icon={Boxes}
            trend={null} // Backend doesn't provide trend history yet
            color="success"
          />
          <KPICard
        ...
        <div className="bg-white p-4 sm:p-5 lg:p-6 rounded-xl shadow-sm border border-[#D7CCC8] mb-6">
          <div className="mb-4 sm:mb-5">
            <h2 className="text-base sm:text-lg font-semibold text-[#3E2723]">
              Quick Actions
            </h2>
          </div>
